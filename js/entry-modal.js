...
function openEntryModal(stage, itemId, entryId){
  if(entryId && !isAdmin()){ showToast('Hanya Admin yang bisa mengedit transaksi. Divisi lain hanya bisa mencatat transaksi baru.'); return; }
...
async function deleteEntry(){
  if(!isAdmin()){ showToast('Hanya Admin yang bisa menghapus transaksi'); return; }
  const stage = document.getElementById('e_stage').value;
  const id = document.getElementById('e_id').value;
  if(!id) return;

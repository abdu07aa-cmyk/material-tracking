...
/* ---------- Insight helpers (additive — used by Dashboard "Perlu Perhatian" & sort) ---------- */

// Barang dianggap butuh perhatian kalau sudah lebih dari N hari sejak transaksi
// terakhir, tapi masih ada sisa di salah satu tahap (belum tuntas ke Produksi).
const ATTENTION_STALE_DAYS = 3;

function lastActivityDate(s){
  const dates = [s.lastReceiptDate, s.lastQcGudangDate, s.lastQcDiecutDate, s.lastDiecutDate, s.lastSupplyDate, s.lastProductionDate].filter(Boolean);
...
// Tahap mana yang jadi "penyumbat" saat ini, mengikuti urutan alur barang.
function currentBottleneck(s){
  if(s.remainToQcGudang>0) return {label:'QC Awal', tab:'qc'};
  if(s.remainToDiecut>0) return {label:'Diecut', tab:'diecut'};
  if(s.remainToQcDiecut>0) return {label:'QC Ulang', tab:'qc'};
  if(s.remainToSupply>0) return {label:'Suplai', tab:'suplai'};
  if(s.remainToProduction>0) return {label:'Produksi', tab:'produksi'};
  return null;
}
...
function computeAttentionItems(){
  ...
  items.forEach(it=>{
    ...
    const bottleneck = currentBottleneck(s);
    if(!bottleneck) return; // sudah tuntas atau belum ada aktivitas sama sekali
    const lastDate = lastActivityDate(s);
    const days = lastDate ? daysSince(lastDate) : null;
    // Barang yang belum pernah disentuh sama sekali (baru dibuat, tanpa lastDate)
    // dianggap "menunggu" sejak awal dan tetap masuk daftar dengan label khusus.
    if(days===null || days>=ATTENTION_STALE_DAYS){
      out.push({ item:it, bottleneck, days });
    }
  });
  out.sort((a,b)=> (b.days ?? 9999) - (a.days ?? 9999));
  return out;
}

// Metrik qty yang relevan untuk tiap tab — dipakai oleh opsi sortir "Sisa/Qty Terbanyak".
function sortMetric(s, stage){
  if(stage==='gudang') return s.receivedQty;
  if(stage==='qc') return s.remainToQc;
  if(stage==='diecut') return s.remainToDiecut;
  if(stage==='suplai') return s.remainToSupply;
  if(stage==='produksi') return s.remainToProduction;
  return 0;
}

function sortItemsForStage(list, stage, sortVal){
  const arr = list.slice();
  if(sortVal==='pi_terbaru') arr.sort((a,b)=> (b.pi_date||'').localeCompare(a.pi_date||''));
  else if(sortVal==='pi_terlama') arr.sort((a,b)=> (a.pi_date||'').localeCompare(b.pi_date||''));
  else if(sortVal==='qty_terbanyak') arr.sort((a,b)=> sortMetric(itemStats(b), stage) - sortMetric(itemStats(a), stage));
  return arr;
}
